...
import static aegis.com.aegis.app.Storage.EXT_AAC;
...
public class RecordingService extends Service implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static boolean isEnabled(Context context) {
        ...
        if (!Storage.permitted(context, MUST))
            b = false;
        return b;
    }
    ...
    class PhoneStateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (a.equals(TelephonyManager.ACTION_PHONE_STATE_CHANGED)) {
                setPhone(intent.getStringExtra(TelephonyManager.EXTRA_INCOMING_NUMBER), callType);
            }
    ...
    class PhoneStateChangeListener extends PhoneStateListener {
        ...
        public void onCallStateChanged(final int s, final String incomingNumber) {
            try {
                Log.d(TAG, "onCallStateChanged() called with: s = [" + s + "], incomingNumber = [" + incomingNumber + "]");
                switch (s) {
                    ...
                    case TelephonyManager.CALL_STATE_OFFHOOK:
                        ...
                        setPhone(incomingNumber, callType);
                        if (thread == null) { // handling restart while current callType
                            begin(wasRinging);
                            startedByCall = true;
                        }
                        break;
                    case TelephonyManager.CALL_STATE_IDLE:
                        if (startedByCall) {
                            if (tm.getCallState() != TelephonyManager.CALL_STATE_OFFHOOK) { // current state maybe differed from queued (s) one
                                ...
                                return; // fast clicking. new callType already stared. keep recording. do not reset startedByCall
                            }
                        } else {
                            if (storage.recordingPending()) { // handling restart after callType finished
                                finish();
                            } else if (storage.recordingNextPending()) { // only callType encodeNext if we have next encoding
                                encodingNext();
                            }
                        }
                        wasRinging = false;
    ...
    public void setPhone(String s, String c) {
        Log.d(TAG, "setPhone() called with: s = [" + s + "], c = [" + c + "]");
        if (s == null || s.isEmpty())
            return;
        ...
        if (Storage.permitted(this, CONTACTS)) {
            try {
...
//                MainActivity.startActivity(this);
    ...
    public String getSourceText() {
        Log.d(TAG, "getSourceText() called");
        switch (source) {
    ...
    public Notification buildNotification(Notification when) {
        Log.d(TAG, "buildNotification() called with: when = [" + when + "]");
    ...
    public Notification buildPersistent(Notification when) {
        Log.d(TAG, "buildPersistent() called with: when = [" + when + "]");
        PendingIntent main = PendingIntent.getActivity(this, 0
                , new Intent(this, MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
        NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        String channelId = getString(R.string.app_name);
    ...
    public void showNotificationAlarm(boolean show) {
        Log.d(TAG, "showNotificationAlarm() called with: show = [" + show + "]");
        boolean recording = thread != null;
//        MainActivity.showProgress(RecordingService.this, show, phone, samplesTime / sampleRate, recording);
    ...
    public void showDone(Uri targetUri) {
        Log.d(TAG, "showDone() called with: targetUri = [" + targetUri + "]");
        final SharedPreferences shared = PreferenceManager.getDefaultSharedPreferences(this);
        RecordingService.targetUri = targetUri;
        EventBus.getDefault().postSticky(new RecordingData(targetUri, phone, contactName));
    }
    ...
    void startAudioRecorder(int[] ss, int i) {
        Log.d(TAG, "startAudioRecorder() called with: ss = [" + ss + "], i = [" + i + "]");
        final CallInfo info = new CallInfo(targetUri, phone, contactName, contactId, callType, now);
    ...
    void startMediaRecorder(String ext, int[] ss, int i) {
        Log.d(TAG, "startMediaRecorder() called with: ext = [" + ext + "], ss = [" + ss + "], i = [" + i + "]");
        try {
            final CallInfo info = new CallInfo(targetUri, phone, contactName, contactId, callType, now);
            FileDescriptor fd;
...
//                            MainActivity.showProgress(RecordingService.this, true, phone, samplesTime / sampleRate, null);
    ...
    void encoding(final File in, final Uri uri, final Runnable done, final Success success) {
        Log.d(TAG, "encoding() called with: in = [" + in + "], uri = [" + uri + "], done = [" + done + "], success = [" + success + "]");
...
//                MainActivity.setProgress(RecordingService.this, encoder.getProgress());
            ...
            @Override
            public void run() {  // success only callType, done
                save.run();
            }
        }, new Runnable() {
            @Override
...
//                MainActivity.showProgress(RecordingService.this, false, phone, samplesTime / sampleRate, false);
    ...
    void Post(Throwable e) {
        Log.d(TAG, "Post() called with: e = [" + e + "]");
...
//        MainActivity.showProgress(this, true, phone, samplesTime / sampleRate, thread != null);
    ...
    void begin(boolean wasRinging) {
        Log.d(TAG, "begin() called with: wasRinging = [" + wasRinging + "]");
        now = System.currentTimeMillis();
        targetUri = storage.getNewFile(now, phone, contactName, callType);
        if (encoder != null) {
            encoder.pause();
        }
    ...
    void finish() {
        ...
        if (tmp.exists() && tmp.length() > 0) {
            ...
            Storage.move(tmp, in);
            mapTarget.put(in, new CallInfo(targetUri, phone, contactName, contactId, callType, now));
